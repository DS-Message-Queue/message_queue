...
class Transport(object):
    ...
    def __init__(self, syncobj, selfNode, otherNodes):
        ...
        self._onMessageReceivedCallbacks = {}
        self._onNodeConnectedCallbacks = []
        self._onNodeDisconnectedCallbacks = []
        self._onReadonlyNodeConnectedCallback = None
        self._onReadonlyNodeDisconnectedCallback = None
        self._onUtilityMessageCallbacks = {}

    def setOnMessageReceivedCallback(self, topic_partition, callback):
        """
    ...
    def setOnNodeConnectedCallback(self, callback):
        ...
        self._onNodeConnectedCallbacks.append(callback)
    ...
    def setOnNodeDisconnectedCallback(self, callback):
        ...
        self._onNodeDisconnectedCallbacks.append(callback)
    ...
    def _onMessageReceived(self, node, message):
        ...
        if message['partition'] in self._onMessageReceivedCallbacks:
            self._onMessageReceivedCallbacks[message['partition']](node, message)

    def _onNodeConnected(self, node):
        print(node, 'connection.')
        for callback in self._onNodeConnectedCallbacks:
            callback(node)

    def _onNodeDisconnected(self, node):
        print(node, 'disconnection.')
        for callback in self._onNodeDisconnectedCallbacks:
            callback(node)
...
class TCPTransport(Transport):
    def __init__(self, poller, conf, selfNode, otherNodes):
        """
    ...
    def _createServer(self):
        ...
        self._server = TcpServer(self._poller, host, port, onNewConnection = self._onNewIncomingConnection,
    ...
    def _maybeBind(self):
        ...
        if self._ready or self._selfIsReadonlyNode or monotonicTime() < self._lastBindAttemptTime + self._conf.bindRetryTime:
            return
        ...
        try:
            ...
            if self._conf.maxBindRetries and self._bindAttempts >= self._conf.maxBindRetries:
    ...
    def _onIncomingMessageReceived(self, conn, message):
        ...
        # no encryption
        encryptor = None
        if encryptor and not conn.sendRandKey:
    ...
    def _connectIfNecessarySingle(self, node):
        ...
        if node in self._lastConnectAttempt and monotonicTime() - self._lastConnectAttempt[node] < self._conf.connectionRetryTime:
            return False
    ...
    def addNode(self, node):
        ...
        if self._shouldConnect(node):
            conn = TcpConnection(
                poller = self._poller,
                timeout = self._conf.connectionTimeout,
                sendBufferSize = self._conf.sendBufferSize,
                recvBufferSize = self._conf.recvBufferSize,
                keepalive = self._conf.tcp_keepalive,
            )
