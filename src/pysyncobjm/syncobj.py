...
class SyncObj(object):
    def __init__(self, topic_partition, selfNode, otherNodes, conf=None, consumers=None, nodeClass = TCPNode, transport = None, transportClass = TCPTransport):
        ...
        # poller is a common object
        # self._poller = poller

        # set the partition
        self.__partition = topic_partition
        ...
        self.__transport.setOnMessageReceivedCallback(topic_partition, self.__onMessageReceived)
    ...
    def __onNodeConnected(self, node):
        # add only if the node is part of this network
        if node in self.__otherNodes | {self.__selfNode}:
            self.__connectedNodes.add(node)
